...
# def generate_matches(team_names, matches_per_team=4):
#     matches = []
#     teams = [make_team(t) for t in team_names]
#     team_play_counts = {name: 0 for name in team_names}
#     paired = {name: {} for name in team_names}
...
#     while min(team_play_counts.values()) < matches_per_team:
#         # pick 6 teams that still have available matches
#         available_teams = [name for name in team_names if team_play_counts[name] < matches_per_team]

#         if len(available_teams) < 6:
#             break  # not enough teams to form a match

#         # try to pick 3 teams for red alliance with no repeated teammates
#         random.shuffle(available_teams)
#         red = []
#         for t in available_teams:
#             if all(t not in paired.get(r, {}) for r in red):
#                 red.append(t)
#             if len(red) == 3:
#                 break
...
#         blue = []
#         for t in available_teams:
#             if all(t not in paired.get(b, {}) for b in blue):
#                 blue.append(t)
#             if len(blue) == 3:
#                 break

#         # if either alliance is not full, reshuffle and retry
...
#                     if i != j:
#                         paired[alliance[i]][alliance[j]] = True
